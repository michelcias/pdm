...
#'
#' @return An object of class \code{c("binomial_localtrend", "bdm_mcmc", "list")}
...
#' @keywords internal
#' @noRd
new_binomial_localtrend <- function(result,
                                    n_obs,
                                    n_chain,
                                    burnin,
                                    thinning,
  ...
  # Add class hierarchy
  class(result) <- c("binomial_localtrend", "bdm_mcmc", "list")
...
validate_binomial_localtrend <- function(x) {
  ...
  # 1. Check class
  if (!inherits(x, "binomial_localtrend")) {
    stop("Object must inherit from class 'binomial_localtrend'")
  }
...
is.binomial_localtrend <- function(x) {
  inherits(x, "binomial_localtrend")
}
...
print.binomial_localtrend <- function(x, digits = 3, ...) {
  ...
  cat("\n")
  cat("Binomial Local-Trend Model (Logit Link)\n")
  cat(strrep("=", 70), "\n\n", sep = "")
