#' Constructor for poisson_localacceleration class
#'
#' @description Internal constructor function for creating objects of class
#'   `poisson_localacceleration`. This function is called by
#'   \code{\link{mcmc_poisson_localacceleration}} and should not be called directly by
#'   users.
#'
...
#'   \itemize{
#'     \item `poisson_localacceleration`:  Specific model class
...
#' @keywords internal
#' @noRd
new_poisson_localacceleration <- function(result,
                                          n_obs,
                                          n_draws,
                                          burnin,
                                          thinning,
  ...
  # Add class hierarchy
  class(result) <- c("poisson_localacceleration", "pdm_mcmc", "list")
...
#' Validator for poisson_localacceleration class
#'
#' @description Internal function to validate objects of class
#'   `poisson_localacceleration`.  Checks that all required components are present
...
#' @keywords internal
#' @noRd
validate_poisson_localacceleration <- function(x) {

  # 1. Check class
  if (!inherits(x, "poisson_localacceleration")) {
    stop("Object must inherit from class 'poisson_localacceleration'")
  }
...
#' Check if object is of class poisson_localacceleration
#'
#' @description Test whether an object is of class `poisson_localacceleration`.
#'
#' @param x An object to test.
#'
#' @return Logical value:  `TRUE` if `x` inherits from
#'   `poisson_localacceleration`, `FALSE` otherwise.
#'
#' @examples
#' ## A minimal fit is all this test needs; see
#' ## ?mcmc_poisson_localacceleration for a realistic analysis.
#' set.seed(123)
#' n <- 40
...
#' )
#'
#' is.poisson_localacceleration(out)     # TRUE
#' is.poisson_localacceleration(list())  # FALSE
#'
#' @export
is.poisson_localacceleration <- function(x) {
  inherits(x, "poisson_localacceleration")
}


#' Print method for poisson_localacceleration objects
#'
#' @description Prints a concise summary showing posterior medians.
#'   Use `summary()` for comprehensive statistics when available.
#'
#' @param x An object of class `poisson_localacceleration`.
...
#' n <- 200
#'
#' # Generate true rates with acceleration
...
print.poisson_localacceleration <- function(x, digits = 3, ...) {
  ...
  # Validate input
  if (!inherits(x, "poisson_localacceleration")) {
    stop("Object must be of class 'poisson_localacceleration'")
  }
  ...
  cat("\n")
  cat("Poisson Local-Acceleration Model (Log Link)\n")
  cat(strrep("=", 70), "\n\n", sep = "")
  ...
  tryCatch({
    ...
    cat("Latent Acceleration (theta_{t,3}) Median Summary (log scale):\n")
    cat("  Range:  [",
