#' Constructor for poisson_mixture_locallevel class
#'
#' @description Internal constructor function for creating objects of class
#'   `poisson_mixture_locallevel`. This function is called by
#'   \code{\link{mcmc_poisson_mixture_locallevel}} and should not be called
#'   directly by users.
#'
...
#'
#' @return An object of class `c("poisson_mixture_locallevel", "pdm_mcmc", "list")`
#'   with the following structure:
#'   \describe{
#'     \item{Data components}{All elements from `result` (lambda_1, lambda_2,
#'       theta_1, theta_01, prec_theta1, alpha, z, and optionally log_sigma and
#'       accept_prop)}
#'     \item{Attributes}{
#'       \itemize{
...
#'   \itemize{
#'     \item `poisson_mixture_locallevel`: Specific model class
...
#' @keywords internal
#' @noRd
new_poisson_mixture_locallevel <- function(result,
                                           link,
                                           n_obs,
                                           n_draws,
                                           burnin,
  ...
  # Add class hierarchy
  class(result) <- c("poisson_mixture_locallevel", "pdm_mcmc", "list")
...
#' Validator for poisson_mixture_locallevel class
#'
#' @description Internal function to validate objects of class
#'   `poisson_mixture_locallevel`. Checks that all required components
...
#' @keywords internal
#' @noRd
validate_poisson_mixture_locallevel <- function(x) {
  validate_poisson_mixture(x, "poisson_mixture_locallevel", order = 1L)
}


#' Check if object is of class poisson_mixture_locallevel
#'
#' @description Test whether an object is of class `poisson_mixture_locallevel`.
#'
#' @param x An object to test.
#'
#' @return Logical value: `TRUE` if `x` inherits from
#'   `poisson_mixture_locallevel`, `FALSE` otherwise.
#'
#' @examples
#' ## A minimal fit is all this test needs; see
#' ## ?mcmc_poisson_mixture_locallevel for a realistic analysis.
#' set.seed(123)
#' n <- 40
...
#' )
#'
#' is.poisson_mixture_locallevel(out)     # TRUE
#' is.poisson_mixture_locallevel(list())  # FALSE
#'
#' @export
is.poisson_mixture_locallevel <- function(x) {
  inherits(x, "poisson_mixture_locallevel")
}


#' Print method for poisson_mixture_locallevel objects
#'
...
#'
#' @param x An object of class `poisson_mixture_locallevel`.
...
#'   \code{\link{plot.poisson_mixture_locallevel}}, \code{\link{summary.poisson_mixture_locallevel}}.
#'
#' @export
print.poisson_mixture_locallevel <- function(x, digits = 3, ...) {
  print_poisson_mixture(x, "poisson_mixture_locallevel", digits, order = 1L)
}
