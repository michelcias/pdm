#' Constructor for poisson_mixture_localtrend class
#'
#' @description Internal constructor function for creating objects of class
#'   `poisson_mixture_localtrend`. This function is called by
#'   \code{\link{mcmc_poisson_mixture_localtrend}} and should not be called
#'   directly by users.
#'
...
#'
#' @return An object of class `c("poisson_mixture_localtrend", "pdm_mcmc", "list")`
#'   with the following structure:
#'   \describe{
#'     \item{Data components}{All elements from `result` (lambda_1, lambda_2,
#'       theta_1, theta_2, theta_01, theta_02, prec_theta1, prec_theta2, alpha,
#'       z, and optionally log_sigma and accept_prop)}
#'     \item{Attributes}{
#'       \itemize{
...
#'   \itemize{
#'     \item `poisson_mixture_localtrend`: Specific model class
...
#' @keywords internal
#' @noRd
new_poisson_mixture_localtrend <- function(result,
                                           link,
                                           n_obs,
                                           n_draws,
                                           burnin,
  ...
  # Add class hierarchy
  class(result) <- c("poisson_mixture_localtrend", "pdm_mcmc", "list")
...
#' Validator for poisson_mixture_localtrend class
#'
#' @description Internal function to validate objects of class
#'   `poisson_mixture_localtrend`. Checks that all required components
...
#' @keywords internal
#' @noRd
validate_poisson_mixture_localtrend <- function(x) {
  validate_poisson_mixture(x, "poisson_mixture_localtrend", order = 2L)
}


#' Check if object is of class poisson_mixture_localtrend
#'
#' @description Test whether an object is of class `poisson_mixture_localtrend`.
#'
#' @param x An object to test.
#'
#' @return Logical value: `TRUE` if `x` inherits from
#'   `poisson_mixture_localtrend`, `FALSE` otherwise.
#'
#' @examples
#' ## A minimal fit is all this test needs; see
#' ## ?mcmc_poisson_mixture_localtrend for a realistic analysis.
#' set.seed(123)
#' n <- 40
#' z <- rbinom(n, 1, plogis(cumsum(rnorm(n, sd = 0.2))))
#' y <- rpois(n, ifelse(z == 1, 8, 1))
#'
#' out <- mcmc_poisson_mixture_localtrend(
#'   y,
#'   link     = "logit",
#'   burnin   = 50,
#'   thinning = 1,
...
#' )
#'
#' is.poisson_mixture_localtrend(out)     # TRUE
#' is.poisson_mixture_localtrend(list())  # FALSE
#'
#' @export
is.poisson_mixture_localtrend <- function(x) {
  inherits(x, "poisson_mixture_localtrend")
}


#' Print method for poisson_mixture_localtrend objects
#'
...
#'
#' @param x An object of class `poisson_mixture_localtrend`.
...
#'   \code{\link{plot.poisson_mixture_localtrend}}, \code{\link{summary.poisson_mixture_localtrend}}.
#'
#' @export
print.poisson_mixture_localtrend <- function(x, digits = 3, ...) {
  print_poisson_mixture(x, "poisson_mixture_localtrend", digits, order = 2L)
}
