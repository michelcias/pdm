...
#'   The names this model accepts are `theta_01` and `prec_theta1`;
...
#'
#' @seealso
#'   \code{\link{plot.binomial_locallevel}},
#'   \code{\link{print.binomial_locallevel}},
#'   \code{\link{summary.binomial_locallevel}} for methods on the fitted object;
#'   \code{\link{mcmc_binomial_localtrend}} and
...
mcmc_binomial_locallevel <- function(y,
  ...
  # Record whether the user explicitly set the degrees of freedom (used to flag a
  ...
  # Resolve the innovation precision prior (Gamma or Half-t). Validation and the
  # "halfcauchy" -> Half-t(df = 1) normalisation are handled by the shared
  # internal helper `resolve_prec_prior()` (see R/prec_prior.R).
  ...
  # Record the prior actually used on the innovation precision (for
