...
#'   The names this model accepts are `theta_01`, `theta_02`, `theta_03`,
#'   `prec_theta1`, `prec_theta2`, `prec_theta3` and `prec_y`; any subset may be
#'   given, and precisions must be positive. An unrecognised name is an error
#'   rather than being ignored, so a misspelling cannot pass for a starting
#'   value that quietly had no effect. With `chains > 1` pass a list of `chains`
#'   such lists, one per chain: a single starting point shared by every chain
#'   removes the dispersion the Gelman-Rubin statistic in
...
#'   \item{\code{prec_theta3}}{Numeric vector of length `n_draws` for the acceleration innovation precision \eqn{1/W_3}.}
...
#' # journey back. The other parameters are left out, so they are still drawn.
