...
#'   [mcmc_normal_mixture_locallevel()] and should not be called directly by users.
#'
...
#'   with result components and relevant metadata stored as attributes.
#' @keywords internal
#' @noRd
new_normal_mixture_locallevel <- function(result,
  ...
  attr(result, "model_type") <- "locallevel"
  attr(result, "y") <- y
...
validate_normal_mixture_locallevel <- function(x) {
  ...
  required_components <- c(
    ...
    "theta_1", "theta_01", "prec_theta1", "alpha", "z"
  )
  ...
  scalar_params <- c("mu_1", "mu_2", "prec_1", "prec_2", "theta_01", "prec_theta1")
