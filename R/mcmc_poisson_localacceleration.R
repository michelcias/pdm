#' @title Gibbs Sampler for a Local-Acceleration Poisson Dynamic Model
#'
#' @description Runs a Gibbs sampler for the local-acceleration Poisson dynamic model
#'   with log link.
#'
...
#'
#' Each innovation precision (\eqn{W_1^{-1}}, \eqn{W_2^{-1}}, \eqn{W_3^{-1}}) may
#' take one of two priors, chosen independently through `prior_prec1_type`,
#' `prior_prec2_type` and `prior_prec3_type`:
#'
...
#'   \eqn{\eta_2} (rate), \eqn{\nu_2} (df) \tab `prior_prec2_rate`, `prior_prec2_df` \cr
#'   \eqn{\nu_3} (shape), \eqn{A_3} (scale) \tab `prior_prec3_shape`, `prior_prec3_scale` \cr
#'   \eqn{\eta_3} (rate), \eqn{\nu_3} (df) \tab `prior_prec3_rate`, `prior_prec3_df`
#' }
#'
...
#'   precision \eqn{1/W_3}: `"halfcauchy"` (default) or `"halft"` for a Half-t /
#'   Half-Cauchy prior on \eqn{\sqrt{W_3}} (Gelman, 2006), or `"gamma"` for a
#'   Gamma prior on the precision. The default changed in 0.5-0; a call that
#'   supplies `prior_prec3_shape`/`prior_prec3_rate` without naming a type is
#'   still read as `"gamma"`, so existing code keeps working.
...
#'   \eqn{\sqrt{W_3}}. Default `2`, on the scale of the link -- a fixed value
#'   is enough here, unlike the Gaussian family, because the link scale carries
#'   no arbitrary units. Ignored when `prior_prec3_type = "gamma"`.
...
#'   with components:
#' \describe{
#'   \item{\code{theta_1}}{Numeric matrix \eqn{[n_{draws} \times n]} of posterior samples for \eqn{\theta_{t,1}}.}
#'   \item{\code{theta_2}}{Numeric matrix \eqn{[n_{draws} \times n]} of posterior samples for \eqn{\theta_{t,2}}.}
#'   \item{\code{theta_3}}{Numeric matrix \eqn{[n_{draws} \times n]} of posterior samples for \eqn{\theta_{t,3}}.}
...
#'   \item{\code{theta_03}}{Numeric vector of length `n_draws` of posterior samples for \eqn{\theta_{0,3}}.}
...
#'   \item{\code{prec_theta3}}{Numeric vector of length `n_draws` of posterior samples for \eqn{1/W_3}.}
#'   \item{\code{alpha}}{Numeric matrix \eqn{[n_{draws} \times n]} of posterior samples for \eqn{\alpha_t} (rates). Convergence of \eqn{\alpha_t} is already covered by the
...
#'   \item{\code{log_sigma}}{(Optional) Numeric matrix \eqn{[n_{draws} \times n]} of proposal scale diagnostics
#'     (if `return_log_sigma = TRUE`).}
#'   \item{\code{accept_prop}}{(Optional) Numeric matrix \eqn{[n_{draws} \times n]} of acceptance proportion
...
#' # 1. Simulate data from a local-acceleration Poisson dynamic model
#' # 2. Use `mcmc_poisson_localacceleration` to estimate parameters and latent states
...
#'
#' # Generate true rates with acceleration (ensuring positive values)
#' grid_vals <- seq_len(n) / n
#' alpha_true <- exp(sin(4 * pi * grid_vals) + sin(8 * pi * grid_vals) + 1)
#'
...
#' # the scale of `y`: hence log(mean(y)) rather than mean(y). `prec_theta1` is
...
#' out_hc <- mcmc_poisson_localacceleration(
#'   y,
#'   burnin             = 1000,
#'   thinning           = 20,
#'   n_draws            = 500,
...
#'   # (`out_hc` was run with return_accept_prop = FALSE, so the acceptance
#'   # diagnostic is unavailable for it and is omitted below.)
#'   plot(out_hc, type = "all")
#'
#'   plot(out_hc, type = "mcmc", which = 1:3)  # Parameter diagnostics
#'   plot(out_hc, type = "states")             # Dynamic states
#'   plot(out_hc, type = "alpha")              # Poisson rates
#'
#'   # Plot with true alpha for comparison
...
#'
#' @seealso
#'   \code{\link{plot.poisson_localacceleration}},
#'   \code{\link{print.poisson_localacceleration}},
#'   \code{\link{summary.poisson_localacceleration}} for methods on the fitted object;
#'   \code{\link{mcmc_poisson_locallevel}} and
#'   \code{\link{mcmc_poisson_localtrend}} for the other dynamic orders.
#'
#' @export
mcmc_poisson_localacceleration <- function(y,
                                           burnin,
                                           thinning,
                                           n_draws,
                                           prior_theta01_mean,
  ...
  # Call the C function
  result <- .Call(
    "_pdm_C_MCMC_log_poisson_localacceleration",
    as.numeric(y),
    as.integer(burnin),
    as.integer(thinning),
    as.integer(n_draws),
  ...
  result <- new_poisson_localacceleration(
    result = result,
    n_obs = length(y),
    n_draws = n_draws,
    burnin = burnin,
