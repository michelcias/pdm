#' @title Gibbs Sampler for a Poisson Mixture Model with Local-Acceleration Mixture Weights
#'
#' @description Runs a Gibbs sampler for a two-component Poisson mixture model
#'   with time-varying mixture weights governed by a local-acceleration dynamic
...
#' \code{\link{mcmc_poisson_localacceleration}} both put a Poisson observation
#' on a dynamic model, and they are not variants of one another. There the latent
...
#'
#' \emph{Initial states:}
#' \deqn{\theta_{0,j} \sim N(\mu_{\theta_{0j}}, \tau_{\theta_{0j}}^{-1}), \quad j = 1, 2, 3.}
#'
#' \emph{State innovation precisions:}
#'
#' Each of \eqn{W_1^{-1}}, \eqn{W_2^{-1}} and \eqn{W_3^{-1}} may take one of two
#' priors, chosen independently through the corresponding `*_type` argument:
#'
#' \emph{(a) Gamma prior on the precision} (conjugate):
#' \deqn{W_j^{-1} \sim \text{Gamma}(\nu_j, \eta_j), \quad j = 1, 2, 3.}
#'
#' \emph{(b) Half-t prior on the standard deviation} (Gelman, 2006; the default):
#' \deqn{\sqrt{W_j} \sim \text{Half-}t(\nu_j, A_j), \quad j = 1, 2, 3,}
...
#'   \eqn{\mu_{\theta_{0j}}} \tab `prior_theta0j_mean` \cr
#'   \eqn{\tau_{\theta_{0j}}} \tab `prior_theta0j_prec` \cr
#'   \eqn{\nu_j} (shape), \eqn{A_j} (scale) \tab `prior_precj_shape`, `prior_precj_scale` \cr
#'   \eqn{\eta_j} (rate), \eqn{\nu_j} (df) \tab `prior_precj_rate`, `prior_precj_df`
#' }
#'
...
#' @param prior_theta03_mean Numeric, prior mean for the initial acceleration
#'   state \eqn{\theta_{0,3}}. Default is 0.
...
#'   precision \eqn{1/W_3} (`"halfcauchy"` default, or `"halft"` / `"gamma"`).
#' @param prior_prec3_shape Numeric > 0, shape parameter of the Gamma prior for
#'   the acceleration innovation precision \eqn{1/W_3}. Default is 0.01 (vague
#'   prior). Used only when `prior_prec3_type = "gamma"`.
#' @param prior_prec3_rate Numeric > 0, rate parameter of the Gamma prior for
#'   \eqn{1/W_3}. Default is 0.01 (vague prior). Used only when
#'   `prior_prec3_type = "gamma"`.
#' @param prior_prec3_scale Numeric > 0, Half-t scale for \eqn{\sqrt{W_3}}.
#'   Default `2`. Ignored when `prior_prec3_type = "gamma"`.
#' @param prior_prec3_df Numeric > 0, Half-t df \eqn{\nu_3} for \eqn{\sqrt{W_3}}.
#'   Default `1` (Half-Cauchy).
...
#'   `theta_02`, `theta_03`, `prec_theta1`, `prec_theta2` and `prec_theta3`;
...
#'   \item{\code{theta_3}}{Numeric matrix \eqn{[n_{draws} \times n]} of posterior
...
#' # 1. Simulate counts from a Poisson mixture with curving mixture weights
#' # 2. Use `mcmc_poisson_mixture_localacceleration` to estimate the model
...
#' out_probit <- mcmc_poisson_mixture_localacceleration(
#'   y,
#'   link               = "probit",
#'   burnin             = 1000,
#'   thinning           = 10,
...
#' )
#'
#' ## Alternative: weakly-informative Half-Cauchy priors on all three innovations
#' # All three weight-innovation SDs live on the probit latent scale, so a scale
#' # of about 1 is weakly informative there. The component rates keep their
...
#' out_hc <- mcmc_poisson_mixture_localacceleration(
#'   y,
#'   link               = "probit",
#'   burnin             = 1000,
#'   thinning           = 10,
...
#'   plot(out_logit, type = "mcmc", which = 3:8)   # theta_0 and W^-1
...
#'   \code{\link{summary.poisson_mixture_localacceleration}} for methods on the fitted object;
#'   \code{\link{mcmc_poisson_mixture_locallevel}} and
#'   \code{\link{mcmc_poisson_mixture_localtrend}} for the other dynamic orders.
#'
#' @export
mcmc_poisson_mixture_localacceleration <- function(y,
                                                   link = c("logit", "probit"),
                                                   burnin,
                                                   thinning,
                                                   n_draws,
  ...
  result <- .Call(
    "_pdm_C_MCMC_poisson_mixture_localacceleration",
    as.numeric(y),
    as.character(link),
    as.integer(burnin),
    as.integer(thinning),
  ...
  result <- new_poisson_mixture_localacceleration(
    result = result,
    link = link,
    n_obs = length(y),
    n_draws = n_draws,
  ...
  result <- validate_poisson_mixture_localacceleration(result)
