#' @title Gibbs Sampler for a Poisson Mixture Model with Local-Level Mixture Weights
#'
...
#' \code{\link{mcmc_poisson_locallevel}} both put a Poisson observation on a
...
#'
#' \emph{Initial state:}
#' \deqn{\theta_{0,1} \sim N(\mu_{\theta_{01}}, \tau_{\theta_{01}}^{-1}).}
#'
#' \emph{State innovation precision:}
#'
#' \eqn{W_1^{-1}} may take one of two priors, chosen through `prior_prec1_type`:
#'
...
#'   The names this model accepts are `lambda_1`, `lambda_2`, `theta_01` and
#'   `prec_theta1`;
...
#' # 1. Simulate counts from a Poisson mixture with sinusoidal mixture weights
#' # 2. Use `mcmc_poisson_mixture_locallevel` to estimate rates and latent states
...
#' out_probit <- mcmc_poisson_mixture_locallevel(
#'   y,
#'   link               = "probit",
#'   burnin             = 1000,
#'   thinning           = 10,
...
#' )
#'
#' ## Alternative: weakly-informative Half-Cauchy prior on the weight innovation
#' # The weight-innovation SD sqrt(W[1]) lives on the probit latent scale, so a
#' # scale of about 1 is weakly informative there. The component rates keep
#' # their conjugate Gamma priors, which have no Half-t option.
...
#' out_hc <- mcmc_poisson_mixture_locallevel(
#'   y,
#'   link               = "probit",
#'   burnin             = 1000,
#'   thinning           = 10,
...
#'   plot(out_logit, type = "mcmc", which = 3:4)   # theta_01 and W_1^-1
...
#'
#' @seealso
#'   \code{\link{plot.poisson_mixture_locallevel}},
#'   \code{\link{print.poisson_mixture_locallevel}},
#'   \code{\link{summary.poisson_mixture_locallevel}} for methods on the fitted object;
#'   \code{\link{mcmc_poisson_mixture_localtrend}} and
#'   \code{\link{mcmc_poisson_mixture_localacceleration}} for the other dynamic orders.
#'
#' @export
mcmc_poisson_mixture_locallevel <- function(y,
                                            link = c("logit", "probit"),
                                            burnin,
                                            thinning,
                                            n_draws,
  ...
  # Record whether the df was explicitly set (used to flag a contradictory
  ...
  result <- .Call(
    "_pdm_C_MCMC_poisson_mixture_locallevel",
    as.numeric(y),
    as.character(link),
    as.integer(burnin),
    as.integer(thinning),
  ...
  result <- new_poisson_mixture_locallevel(
    result = result,
    link = link,
    n_obs = length(y),
    n_draws = n_draws,
  ...
  result <- validate_poisson_mixture_locallevel(result)
