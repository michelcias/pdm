#' Plot method for binomial_localtrend objects
#'
...
#'   using \code{qlogis(alpha)}. However, \code{theta_2} (trend) cannot be recovered
#'   from \code{alpha_true} alone. It must come from your simulation data.
...
#'   \item Page 7: Success probabilities alpha_t
...
plot.binomial_localtrend <- function(x,
  ...
  if (type == "acceptance" && is.null(x$accept_prop)) {
    stop("Acceptance proportions are not available. ",
         "Re-run mcmc_binomial_localtrend() with return_accept_prop = TRUE.")
  }
