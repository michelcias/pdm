...
#'   the component names returned by \code{\link{mcmc_normal_mixture_locallevel}}.
#'
#'   Accepted elements:
#'   \describe{
...
#'     value specified in `mcmc_normal_mixture_locallevel`)
...
#'   \item Page 8: Dynamic state trajectory and diagnostics
...
#' proportion that was specified when running `mcmc_normal_mixture_locallevel`.
...
#'
#' The `overlay_data` parameter controls the display of observed data in
...
#'   # Weakly informative prior for initial state (centered at 0)
...
#' #    Includes: MCMC diagnostics, mixture parameters, state trajectory, weights, acceptance rates
...
#' # mcmc_normal_mixture_locallevel() - see ?mcmc_normal_mixture_locallevel
#' true_vals <- list(
...
#' plot(out, type = "mcmc", which = 5:6, true_values = true_vals)
#'
...
#'
#' #    Example 4: Only mixture weights (without indicators)
#' plot(out, type = "alpha",
#'      true_values = list(alpha = alpha_true))
#'
#' #    Example 5: Only component indicators
#' plot(out, type = "alpha",
...
#'   \code{\link{print.normal_mixture_locallevel}}, \code{\link{summary.normal_mixture_locallevel}}.
#'
#' @export
plot.normal_mixture_locallevel <- function(x,
  ...
  if (type == "acceptance" && is.null(x$accept_prop)) {
    ...
    stop("Acceptance proportions are not available. ",
         "Re-run mcmc_normal_mixture_locallevel() with return_accept_prop = TRUE.")
  }
