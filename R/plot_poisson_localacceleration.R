...
#'     \item{`"states"`}{Dynamic states (theta_1, theta_2, theta_3 trajectories)}
...
#'   (trend) and `theta_3` (acceleration) cannot be recovered from `alpha`
...
#'     value specified in `mcmc_poisson_localacceleration`)
...
#'   \item Page 10: Poisson rates alpha_t
...
#' proportion that was specified when running `mcmc_poisson_localacceleration`.
...
#' # mcmc_poisson_localacceleration() - see ?mcmc_poisson_localacceleration
#' true_vals <- list(
...
plot.poisson_localacceleration <- function(x,
  ...
  if (type == "acceptance" && is.null(x$accept_prop)) {
    stop("Acceptance proportions are not available.  ",
         "Re-run mcmc_poisson_localacceleration() with return_accept_prop = TRUE.")
  }
