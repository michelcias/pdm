...
#'
#' @param x An object of class `poisson_locallevel`.
...
#'     \item{`"alpha"`}{Poisson rates over time (alpha_t)}
...
#'   the component names returned by \code{\link{mcmc_poisson_locallevel}}.
#'
#'   Accepted elements:
#'   \describe{
...
#'   you can obtain `theta_1` using `log(alpha)`.
#'
...
#' \itemize{
#'   \item Time-varying state trajectory with credible bands (on log scale)
...
#'     value specified in `mcmc_poisson_locallevel`)
...
#'   \item Page 4: Poisson rates alpha_t
...
#' proportion that was specified when running `mcmc_poisson_locallevel`.
...
#'
#' # Generate simple oscillating Poisson rates (mimicking seasonal patterns)
#' alpha_true <- exp(sin(2 * pi * seq_len(n) / n) + 1)
#'
#' # Generate Poisson observations (e.g., number of events over time)
...
#'   # Weakly informative prior for initial state (centered at 0 on log scale)
...
#'
#' # 6. Poisson rates with observed counts overlay
#' #    Red points show observed y_t for model validation
#' plot(out, type = "alpha")
#'
#' # 7. Poisson rates without observed data (cleaner for presentations)
...
#' theta1_true <- numeric(n)  # Level state (on log scale)
#'
...
#' # mcmc_poisson_locallevel() - see ? mcmc_poisson_locallevel
#' true_vals <- list(
...
#'
#' # 6. Poisson rates with true alpha trajectory
...
plot.poisson_locallevel <- function(x,
  ...
  if (type == "acceptance" && is.null(x$accept_prop)) {
    stop("Acceptance proportions are not available. ",
         "Re-run mcmc_poisson_locallevel() with return_accept_prop = TRUE.")
  }
  ...
  switch(type,
         ...
         states = plot_dynamic_states_generic_base(x,
                                                   ...
                                                   true_values = true_values,
                                                   ...),
         alpha = plot_poisson_alpha_base(x,
                                         ci = ci,
                                         ci_level = ci_level,
                                         show_obs = show_obs,
                                         true_alpha = true_values$alpha,
