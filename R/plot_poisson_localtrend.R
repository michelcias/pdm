#' Plot method for poisson_localtrend objects
#'
...
#'   \item Page 7: Poisson rates alpha_t
...
#'
#' # 1. Complete diagnostic dashboard (8 pages)
...
#' prec_theta1_true <- 10       # True level innovation precision (high = smooth)
#' prec_theta2_true <- 100      # True trend innovation precision (very smooth)
#'
...
#' # mcmc_poisson_localtrend() - see ? mcmc_poisson_localtrend
#' true_vals <- list(
...
plot.poisson_localtrend <- function(x,
  ...
  if (type == "acceptance" && is.null(x$accept_prop)) {
    stop("Acceptance proportions are not available.  ",
         "Re-run mcmc_poisson_localtrend() with return_accept_prop = TRUE.")
  }
