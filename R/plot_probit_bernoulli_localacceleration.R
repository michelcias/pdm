...
#'
#' @param x An object of class `probit_bernoulli_localacceleration`.
...
#'   the component names returned by \code{\link{mcmc_probit_bernoulli_localacceleration}}.
#'
#'   Accepted elements:
#'   \describe{
...
#'   `theta_2` (trend) and `theta_3` (acceleration) cannot be recovered
#'   from `alpha` alone and must come from your simulation data.
#'
...
#'
#' # 5. Dynamic state trajectories (theta_1, theta_2, theta_3 on probit scale)
...
#' # mcmc_probit_bernoulli_localacceleration() - see ?mcmc_probit_bernoulli_localacceleration
#' true_vals <- list(
