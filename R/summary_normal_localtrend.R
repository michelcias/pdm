#' Summary method for normal_localtrend objects
#'
...
#'
#' @param object An object of class \code{normal_localtrend}, typically the
#'   result of calling \code{\link{mcmc_normal_localtrend}}.
...
#'
#' @return An object of class \code{summary.normal_localtrend}, which is a list
#'   containing:
#'   \describe{
...
#'       scalar parameters (theta_01, theta_02, W_1^-1, W_2^-1, V^-1)}
...
#'
#' For time-varying parameters like \eqn{\theta_{t,1}} and \eqn{\theta_{t,2}},
...
summary.normal_localtrend <- function(object,
  ...
  # Validate input
  validate_summary_input(object, ci_level, "normal_localtrend")

  # Scalar parameters (initial states and precisions)
  scalar_params <- data.frame(
    Parameter = c("theta_01", "theta_02", "W_1^-1", "W_2^-1", "V^-1"),
    rbind(
  ...
  class(result) <- "summary.normal_localtrend"
  return(result)
}


#' Print method for summary.normal_localtrend objects
#'
#' @description Prints comprehensive posterior statistics in a readable format.
#'
#' @param x An object of class \code{summary.normal_localtrend}, typically the
#'   result of calling \code{summary()} on a \code{normal_localtrend} object.
...
#' }
#'
#' @seealso \code{\link{summary.normal_localtrend}},
#'   \code{\link{print.normal_localtrend}}
#'
#' @export
print.summary.normal_localtrend <- function(x, digits = 3, ...) {

  cat("\n")
  cat("Summary: Gaussian Local-Trend Model\n")
  cat(strrep("=", 75), "\n\n", sep = "")
