 ...
 * @brief Implementation of conditional posterior sampling for initial state parameters
 * @details Provides efficient Bayesian updating algorithms for initial state parameters
 *          in polynomial dynamic models, utilizing Normal-Normal conjugacy for fast
 *          sampling within Gibbs MCMC iterations.
...
void generate_theta_01_locallevel(
  ...
  // Compute the posterior variance: sigma_post^2 = 1/tau_post
  double var_theta_01_post = 1.0 / (prec_theta_01 + prec_theta_1_post[iter]);

  // Compute the posterior mean: mu_post = (mu_01 * tau_01 + theta_1[0] / W_1) / tau_post
