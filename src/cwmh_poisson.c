/**
 * @file cwmh_poisson.c
 * @brief Component-wise Metropolis-Hastings sampling for log-poisson state-space models
 * @author Michel H. Montoril
 * @date 2025-12-20
 * @version 1.0
 *
 * @details This file implements optimized MCMC update routines for state-space models with
 * Poisson observations and log link, including:
 * - Component-wise MH updates for local level (random walk) Poisson models
 * - Component-wise MH updates for local trend (random walk + trend) Poisson models
 ...
 * - Conditional alpha (rate) computation with branch hoisting for optimal performance
 */
...
/**
 * @brief Log-likelihood kernel for Poisson distribution with log link
 * @details Computes the proportional log-likelihood for y ~ Poisson(alpha)
 * where alpha = exp(theta).
 * log(P(y|theta)) \propto y * log(alpha) - alpha
 * \propto y * theta - exp(theta)
 * The term -log(y!) is constant with respect to theta and cancels out
 * in the MH ratio, so it is omitted for efficiency.
 *
 * @param y     Observed count.
 * @param theta Linear predictor (log-rate).
 * @return Proportional log-likelihood value.
 */
static inline double log_lik_poisson(double y, double theta) {
...
typedef struct {
  ...
  int    cached_iter;
} precision_cache_poisson_t;

static precision_cache_poisson_t prec_cache = {-1.0, 0.0, 0.0, -1};
 ...
 * the level state vector theta_1 in a Poisson observation model with log link:
 *
 ...
 * - Inline proportional log-likelihood computation
 ...
 * Will be filled with alpha_t = exp(theta_{t,1}).
 ...
 * save computation time by skipping exp transformations.
 *
 ...
 * @note Numerical stability: Uses optimized log-probabilities.
 ...
 * @note Model specification: Local level Poisson model (random walk, no trend).
 ...
 * @see log_lik_poisson
 */
void cwmh_alpha_log_poisson_locallevel(const double *theta_1_previous,
  ...
  double lp2n = log_lik_poisson(y[0], theta_1_new[0]);
  double lp1o = dnorm(theta_1_previous[0], hat_theta_1[0], sd_regular, 1);
  double lp2o = log_lik_poisson(y[0], theta_1_previous[0]);
  ...
  for (t = 1; t < (n - 1); t++) {
    ...
    lp2n = log_lik_poisson(y[t], theta_1_new[t]);
    lp1o = dnorm(theta_1_previous[t], hat_theta_1[t], sd_regular, 1);
    lp2o = log_lik_poisson(y[t], theta_1_previous[t]);
  ...
  lp2n = log_lik_poisson(y[n - 1], theta_1_new[n - 1]);
  lp1o = dnorm(theta_1_previous[n - 1], hat_theta_1[n - 1], sd_last, 1);
  lp2o = log_lik_poisson(y[n - 1], theta_1_previous[n - 1]);
   ...
   * - compute_alpha = 1: Sequential loop with exp transformations
  ...
  if (compute_alpha) {
    ...
    for (t = 0; t < n; t++) {
      ...
      alpha_current[t] = exp(theta_1_new[t]);     /* Store transformed rates */
    }
  } else {
 ...
 * @note Model specification: Local trend Poisson model (random walk + trend).
 * @note Cache optimization: Precision-dependent calculations are cached between iterations.
 * @note Performance: Branch hoisting and memcpy optimization provide 10-30% speedup
 ...
 * @see log_lik_poisson
 */
void cwmh_alpha_log_poisson(const double *theta_1_previous,
                            double       *theta_1_current,
                            double       *alpha_current,
