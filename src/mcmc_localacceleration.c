/**
 * @file mcmc_localacceleration.c
 * @brief Implementation of MCMC sampling for local-acceleration dynamic models
 * @details Provides a complete Gibbs sampler for Bayesian estimation of polynomial
 *          dynamic models with local-acceleration structure, utilizing conditional posterior
 *          distributions for efficient sampling of level, trend, and acceleration components.
...
SEXP C_MCMC_localacceleration(SEXP y_, SEXP burnin_, SEXP thinning_, SEXP n_chain_,
  ...
  if (len < 3)
    Rf_error("C_MCMC_localacceleration: sample size 'n' must be at least 3, got %lld",
             (long long) len);
  if (len > INT_MAX)
    Rf_error("C_MCMC_localacceleration: sample size too large (%lld > %d)",
  ...
  for (int ii = 1; ii < n_iter; ii++) {
    ...
    generate_precision_theta_p(
      ...
      prec_3_post,   /* prec_theta_p_post: acceleration precision draws */
    ...
    generate_precision_theta_k(
      ...
      theta_3_post,  /* theta_kp1_post: acceleration trajectories */
      prec_2_post,   /* prec_theta_k_post: trend precision draws */
  ...
  SET_VECTOR_ELT(out, 6, prec_1_samples);
  SET_VECTOR_ELT(out, 7, prec_2_samples);
  SET_VECTOR_ELT(out, 8, prec_3_samples);
  ...
  SET_STRING_ELT(nms, 6, mkChar("prec_1"));
  SET_STRING_ELT(nms, 7, mkChar("prec_2"));
  SET_STRING_ELT(nms, 8, mkChar("prec_3"));
