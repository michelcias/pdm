 ...
 *
 *          **Log-Poisson model:**
 *          Observation: y_t ~ Poisson(alpha_t), alpha_t = exp(theta_{t,1})
 ...
 *          - 1/W_1       ~ Gamma(nu_1, eta_1)  (default), or a Half-t prior on
 *            the innovation SD sqrt(W_1) (Gelman, 2006), selected via
 *            prior_prec1_type (see prec_prior_dispatch.h)
 *
...
SEXP C_MCMC_log_poisson_locallevel(SEXP y_,
  ...
  if (len < 3) {
    Rf_error("C_MCMC_log_poisson_locallevel: sample size 'n' must be at least 3, got %lld",
             (long long) len);
  }

  /* Check for integer overflow (R limitation) */
  if (len > INT_MAX) {
    Rf_error("C_MCMC_log_poisson_locallevel: sample size too large (%lld > %d)",
  ...
  for (int t = 0; t < n; t++) {
    if (y[t] < 0 || y[t] != floor(y[t])) {
      Rf_error("C_MCMC_log_poisson_locallevel: y[%d] = %f must be a non-negative integer",
               t, y[t]);
    }
  }
  ...
  /* Innovation precision 1/W_1 prior (no observation precision V in this model).
   * The R wrapper resolves the "halfcauchy" alias to Half-t(df = 1) and passes
   * finite placeholders for the unused fields, so no field is ever NA. */
  ...
  for (int t = 0; t < n; t++) {
    theta_1_previous[t] = 0.0;   /* Zeros for state trajectory (log-rate = 0 -> rate = 1) */
