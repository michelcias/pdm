...
SEXP C_MCMC_log_poisson_localtrend(SEXP y_,
  ...
  if (len < 3) {
    Rf_error("C_MCMC_log_poisson_localtrend: sample size 'n' must be at least 3, got %lld",
             (long long) len);
  }

  /* Check for integer overflow (R limitation) */
  if (len > INT_MAX) {
    Rf_error("C_MCMC_log_poisson_localtrend: sample size too large (%lld > %d)",
  ...
  for (int t = 0; t < n; t++) {
    if (y[t] < 0 || y[t] != floor(y[t])) {
      Rf_error("C_MCMC_log_poisson_localtrend: y[%d] = %f must be a non-negative integer",
               t, y[t]);
    }
  }
  ...
  for (int ii = 1; ii < n_iter; ii++) {
    ...
    /* ===== Step 4: Sample Level State Vector theta_1 and Rates alpha ===== */
