/**
 * @file mcmc_poisson_mixture_localacceleration.h
 * @brief MCMC sampling for Poisson mixture models with dynamic mixture weights
 *        following a local-acceleration structure
 ...
 *          weights evolving as a local-acceleration (level + trend +
 *          acceleration) stochastic process. The dynamic-weight machinery is
 *          shared verbatim with the Gaussian mixture sampler of the same order;
...
#ifndef MCMC_POISSON_MIXTURE_LOCALACCELERATION_H
#define MCMC_POISSON_MIXTURE_LOCALACCELERATION_H
 ...
 *          local-acceleration dynamic structure. Both link functions are served
 ...
 * @param prior_prec3_df_          Double scalar, Half-t df nu_3 > 0 for 1/W_3 (Half-t kind).
 ...
 *                                 Half-t auxiliary per entry of that second block (the two
 *                                 rate slots are 0 and never read). R also enforces
 *                                 lambda_1 <= lambda_2.
 ...
 *         - theta_3:     Matrix [n_draws x n] of acceleration state samples
...
#endif /* MCMC_POISSON_MIXTURE_LOCALACCELERATION_H */
