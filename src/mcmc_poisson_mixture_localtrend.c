 ...
 *          weights that follow a local-trend stochastic process. The dynamic
 *          weight machinery is shared verbatim with the Gaussian mixture
 *          sampler of the same order; only the observation model differs.
 *
 ...
 *
 * **Dynamic weight model (local trend):**
 ...
 * 3. theta_2 | theta_1, theta_{0,2}, W_1, W_2 -> Gaussian posterior (tridiagonal)
 * 4. 1/W_2 | theta_2, theta_{0,2} -> Gamma or Half-t posterior
 * 5. theta_{0,2} | theta_1, theta_2, theta_{0,1}, W_1, W_2 -> Normal posterior
 * 6. theta_1, alpha | z, theta_2, theta_{0,1}, theta_{0,2}, W_1 -> Link-specific
 * 7. 1/W_1 | theta_1, theta_2, theta_{0,1}, theta_{0,2} -> Gamma or Half-t posterior
 * 8. theta_{0,1} | theta_1, theta_{0,2}, W_1 -> Normal posterior
 */
...
/**
 * @brief Unified Gibbs sampler for a Poisson mixture model with local-trend weights
 *
 * @details Executes the full Gibbs sampling cycle for a two-component Poisson
 *          mixture model whose mixture weights follow a local-trend process.
 *          Supports both logit and probit link functions via the `link`
 *          argument.
 *
 ...
 *                                 resolve_init(): the two initial states, then the two
 *                                 component rates and the two state precisions, then one
...
SEXP C_MCMC_poisson_mixture_localtrend(SEXP y_,
  ...
  if (strcmp(link, "logit") != 0 && strcmp(link, "probit") != 0) {
    Rf_error("C_MCMC_poisson_mixture_localtrend: link must be 'logit' or 'probit', got '%s'", link);
  }
  ...
  if (len < 3) {
    Rf_error("C_MCMC_poisson_mixture_localtrend: sample size 'n' must be at least 3, got %lld",
             (long long) len);
  }

  if (len > INT_MAX) {
    Rf_error("C_MCMC_poisson_mixture_localtrend: sample size too large (%lld > %d)",
  ...
  /* Half-t auxiliaries b = 1/a for the state precisions W_1 and W_2 (refreshed
   * when the matching prior is Half-t; left at 0 and never read under the Gamma
   * prior). The component rates carry no auxiliary of their own. */
  ...
  params_current[0]    = init[2];   /* lambda_1 */
  params_current[1]    = init[3];   /* lambda_2 */
  prec_theta1_previous = init[4];
  prec_theta2_previous = init[5];
  /* init[6], init[7] are the (unused) rate auxiliaries. */
  aux_W1               = init[8];
  aux_W2               = init[9];
  ...
  for (int ii = 1; ii < n_iter; ii++) {
    ...
    /* ===== Step 3: Sample Trend State Vector theta_2 ===== */
    /* Draw theta_2 | theta_1, theta_{0,2}, W_2 from a multivariate Normal with
     * tridiagonal precision. Independent of z. */
    generate_theta_p(
    ...
    /* ===== Step 4: Sample Trend Innovation Precision 1/W_2 ===== */
    ...
    /* ===== Step 5: Sample Initial Trend State theta_{0,2} ===== */
    theta_02_current = generate_theta_0p(
      ...
      prec_theta2_current,   /* W_2^{-1}: current trend precision */
      mean_theta02,          /* prior mean */
      prec_theta02,          /* prior precision */
      n                      /* sample size */
    );

    /* ===== Step 6: Sample Level State theta_1 and Mixture Weights alpha ===== */
    ...
    /* ===== Step 7: Sample Level Innovation Precision 1/W_1 ===== */
    ...
    /* ===== Step 8: Sample Initial Level State theta_{0,1} ===== */
