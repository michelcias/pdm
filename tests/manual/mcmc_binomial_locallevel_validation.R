#-------------------------------------------------------------------------------
# Test Script for Binomial Local Level Model (Validation with Helpers)
#
# Objective:
...
#     * adaptation_diagnostics.R
#
# Notes:
# - This script assumes the package native symbols above are registered.
# - If your CWMH wrapper maintains its own internal log_sigma, consider
#   exposing/using a version that accepts an external log_sigma so that
#   adaptation (Section 10) directly influences proposals.
#-------------------------------------------------------------------------------
...
# Generate latent state (random walk on logit scale)
...
accrate_hist    <- matrix(NA_real_, nrow = n_iter, ncol = n)
...
alpha_post[1, ] <- plogis(theta_1_post[1, ])

#-------------------------------------------------------------------------------
# 5) MCMC Loop (C helpers: CWMH + adaptation)
#-------------------------------------------------------------------------------
cat("Starting MCMC loop for Binomial Local Level model...\n")
...
for (ii in 2:n_iter) {
  ...
  # 4) Adaptation (diagnostics/validation)
  #    Note: If your CWMH sampler uses internal log_sigma, this update will only
  ...
  if (lag_update > 0 && ii >= lag_update && (ii %% lag_update == 0)) {
    ...
    accrate      <- as.numeric(adapt_res$accrate)
    log_sigma    <- as.numeric(adapt_res$log_sigma)
    accrate_hist[ii, ]   <- accrate
...
#-------------------------------------------------------------------------------
# 7) Latent State (theta_1 on logit) and Probability (alpha) Summaries
#-------------------------------------------------------------------------------
...
alpha_estimate <- plogis(theta_1_estimate)
alpha_ci_lower <- plogis(theta_1_ci_lower)
alpha_ci_upper <- plogis(theta_1_ci_upper)

# State analysis: generalized helpers (N states)
states_for_analysis <- list(
  theta_1 = list(
    state_name = "theta_1 (logit)",
    true       = theta1_true,
...
# Observed rate vs. probability (optional, scale-consistent)
par(mfrow = c(1, 1), mar = c(4, 4, 3, 1))
plot_observed_vs_state(
  y               = y / n_trials,
  true_state      = alpha_true,
  estimated_state = alpha_estimate,
  title = "Observed rate (y/n_trials) vs. probability (alpha)",
  xlab  = "Time",
  ylab  = "Probability",
  n_plot = 200
)
...
#-------------------------------------------------------------------------------
# 10) Adaptation Diagnostics (acceptance and log_sigma)
#-------------------------------------------------------------------------------
...
cat("=== BINOMIAL LOCAL LEVEL VALIDATION COMPLETED ===\n")
