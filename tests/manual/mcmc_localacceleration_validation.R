#-------------------------------------------------------------------------------
# Test Script for Local Acceleration Model (Validation with Helpers)
#
# Objective:
# - Validate and replicate the Gibbs sampler loop in R for the Gaussian
#   local acceleration model using the C helpers:
#     * _bdm_test_generate_theta_p / _theta_k         (state updates for theta_3/theta_2)
#     * _bdm_test_generate_precision_theta_p / _theta_k (precision updates)
#     * _bdm_test_generate_theta_0p / _theta_0k       (initial state updates)
...
prec_theta03 <- 0.01  # prior_theta03_prec
...
theta_3_post  <- matrix(NA_real_, nrow = n_iter, ncol = n)
...
for (ii in 2:n_iter) {
  ...
  if (length(theta_3_new) == n) {
    ...
    theta_3_post[ii, ] <- theta_3_post[ii-1, ]
  }
  ...
  # 3) initial acceleration theta_03
  ...
  # 4) theta_2 (trend states)
  theta_2_new <- .Call(
    "_bdm_test_generate_theta_k",
  ...
  # 6) initial trend theta_02
  theta_02_post[ii] <- .Call(
    "_bdm_test_generate_theta_0k",
    as.numeric(theta_1_post[ii-1, ]),
    as.numeric(theta_2_post[ii, ]),
...
states_for_analysis <- list(
  ...
  theta_3 = list(
    state_name = "theta_3 (acceleration)",
    true       = theta3_true,
    estimate   = theta_3_estimate,
    ci_lower   = theta_3_ci_lower,
    ci_upper   = theta_3_ci_upper,
    chain      = theta_3_chain
  )
)
...
states_for_plot <- list(
  ...
  theta_3 = list(true = theta3_true, estimate = theta_3_estimate,
                 ci_lower = theta_3_ci_lower, ci_upper = theta_3_ci_upper)
)
...
cat("=== LOCAL ACCELERATION VALIDATION COMPLETED ===\n")
