...
if (requireNamespace("coda", quietly = TRUE)) {
  ...
  prec_chains_mcmc <- coda::mcmc(cbind(prec_1_chain, prec_y_chain))
  colnames(prec_chains_mcmc) <- c("prec_1", "prec_y")
  ...
  heidel_params <- list(theta_01_mcmc, coda::mcmc(prec_1_chain), coda::mcmc(prec_y_chain))
  heidel_names <- c("theta_01", "prec_1", "prec_y")

  for(i in 1:length(heidel_params)) {
    result <- coda::heidel.diag(heidel_params[[i]])
    cat(sprintf("%-10s | %12s | %5d | %9s | %8.4f\n",
                heidel_names[i],
  ...
  cat("Raftery-Lewis Diagnostic (for theta_01):\n")
  cat("========================================\n")
  cat("--- theta_01 ---\n")
  print(coda::raftery.diag(cbind("theta_01" = theta_01_mcmc, prec_chains_mcmc)))
  cat("\n")
