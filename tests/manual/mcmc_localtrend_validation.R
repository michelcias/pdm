...
if (requireNamespace("coda", quietly = TRUE)) {
  ...
  heidel_params <- list(theta_01_mcmc, theta_02_mcmc, prec_1_mcmc, prec_2_mcmc, prec_y_mcmc)
...
# Level + Trend superposition
plot(time_idx, y[time_idx], type = 'p', col = "gray50", pch = 16, cex = 0.5,
     main = "Data vs. Level vs. Trend", xlab = "Time", ylab = "Value",
     ylim = range(c(y[time_idx], theta1_true[time_idx], theta2_true[time_idx],
                    theta_1_estimate[time_idx], theta_2_estimate[time_idx])))
lines(time_idx, theta1_true[time_idx], col = "red", lty = 2, lwd = 2)
lines(time_idx, theta2_true[time_idx], col = "orange", lty = 3, lwd = 2)
lines(time_idx, theta_1_estimate[time_idx], col = "blue", lwd = 1.5)
lines(time_idx, theta_2_estimate[time_idx], col = "darkgreen", lwd = 1.5)
legend("topleft", legend = c("Observed", "True Level", "True Trend", "Est. Level", "Est. Trend"),
       col = c("gray50", "red", "orange", "blue", "darkgreen"),
       pch = c(16, NA, NA, NA, NA),
       lty = c(NA, 2, 3, 1, 1), lwd = c(NA, 2, 2, 1.5, 1.5), bty = "n", cex = 0.8)
