...
# (Gamma vs Half-t / Half-Cauchy) exposed by `mcmc_normal_localtrend()`.

# Small, fast dataset shared across tests.
make_y <- function(n = 40, seed = 1) {
  set.seed(seed)
  cumsum(cumsum(rnorm(n, sd = 0.2))) + rnorm(n, sd = 0.3)
}

run_lt <- function(y, ...) {
  mcmc_normal_localtrend(
    y,
    burnin   = 30,
    thinning = 1,
    n_chain  = 60,
...
test_that("Gamma prior (default) keeps the original interface and output", {
  ...
  expect_s3_class(fit, "normal_localtrend")
  expect_named(fit, c("theta_1", "theta_2", "theta_01", "theta_02",
                      "prec_theta1", "prec_theta2", "prec_y"))
  expect_length(fit$prec_theta1, 60)
  expect_true(all(fit$prec_theta1 > 0))
  expect_true(all(fit$prec_theta2 > 0))
...
test_that("Half-Cauchy prior on all three precisions runs and is recorded", {
  y <- make_y()
...
test_that("Mixed Gamma/Half-t specification (df > 1) runs and is recorded", {
  ...
  fit <- run_lt(
    ...
    prior_prec_y_type  = "halfcauchy", prior_prec_y_scale = 2
  )
...
test_that("invalid prior specifications are rejected with informative errors", {
  ...
  # A Half-t without a scale is no longer an error: derived from the data.
  fit <- run_lt(y,
                prior_prec1_shape = 1e-2, prior_prec1_rate = 1e-2,
                prior_prec2_type  = "halfcauchy",
                prior_prec_y_shape = 1e-2, prior_prec_y_rate = 1e-2)
  expect_equal(attr(fit, "prior_prec_theta2")$scale,
  ...
  # Gamma asked for by name, but shape/rate omitted (on W_1).
  expect_error(
    run_lt(y,
